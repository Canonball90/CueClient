...
@Constant
@ModuleInfo(name = "SearchBar", descriptions = "", category = Category.FUNCTION)
public class ChestSearchBar extends Module {
    ...
    public static final ResourceLocation GENERAL_ICONS_RESOURCE = new ResourceLocation(Cube.MOD_ID, "textures/misc/general_icons.png");
    ...
    public static boolean namesMatch(ItemStack stack, String search)
    {
        ...
        if(tab != null && matcher.matches(I18n.format(tab.getTabLabel()).toLowerCase(), search)) return true;
        if(matcher.matches(name, search)) return true;
