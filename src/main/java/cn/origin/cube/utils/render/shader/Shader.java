...
public abstract class Shader
{
    ...
    public Shader(final String fragmentShader) {
        ...
        try {
            final InputStream vertexStream = this.getClass().getResourceAsStream("/assets/shader/vertex.vert");
            vertexShaderID = this.createShader(IOUtils.toString(Objects.requireNonNull(vertexStream), Charset.defaultCharset()), 35633);
            IOUtils.closeQuietly(vertexStream);
            final InputStream fragmentStream = this.getClass().getResourceAsStream("/assets/shader/" + fragmentShader);
            fragmentShaderID = this.createShader(IOUtils.toString(Objects.requireNonNull(fragmentStream), Charset.defaultCharset()), 35632);
