...
import dev.canon.cue.font.texture.MipmapTexture
...
class AWTFontRenderer(
    ...
    startChar: Int = 0,
    stopChar: Int = 256,
    private var loadingScreen: Boolean = false
) {
...
//        (textureImage.graphics as Graphics2D).drawImage(bufferedImage, 0, 0, null)
        ...
        for (targetChar in startChar until stopChar)
            if (fontImages[targetChar] != null && charLocations[targetChar] != null)
                textureGraphics2D.drawImage(
                    fontImages[targetChar],
                    charLocations[targetChar]!!.x,
                    charLocations[targetChar]!!.y,
                    null
                )
