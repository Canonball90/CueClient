...
import dev.canon.cue.module.modules.visual.ItemPhysics;
import dev.canon.cue.module.modules.visual.ItemPhysics;
        ...
        if (ItemPhysics.INSTANCE.isEnabled()) {
            ...
            for (int j = 0; j < i; ++j) {
                if (ibakedmodel.isAmbientOcclusion()) {
                    GlStateManager.pushMatrix();
                    GlStateManager.scale(ItemPhysics.INSTANCE.scale.getValue().floatValue(), ItemPhysics.INSTANCE.scale.getValue().floatValue(), ItemPhysics.INSTANCE.scale.getValue().floatValue());
