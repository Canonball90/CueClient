package dev.canon.cue.managers
...
class ConfigManager {
    private val path = File(Cue.MOD_NAME)
    ...
    private fun initFriend() {
        ...
        if (!friendFile.exists()) {
            ...
            moduleJson.forEach { Cue.friendManager!!.add(it.toString()) }
        }
    }
    ...
    private fun initCommand() {
        ...
        if (!commandFile.exists()) {
            ...
            commandJson.addProperty("Prefix", Cue.commandPrefix)
            Files.write(
            ...
            Cue.commandPrefix = commandJson.get("Prefix").asString
        }
    }

    private fun saveModuleConfig(module: dev.canon.cue.module.AbstractModule) {
        val modulePath = getModulePath(module)
        if (!modulePath.exists()) {
    ...
    private fun loadModule(module: dev.canon.cue.module.AbstractModule) {
        val modulePath = getModulePath(module)
    ...
    private fun clickGuiJson(): JsonObject {
        ...
        for (panel in Cue.clickGui!!.panels) {
    ...
    private fun hudEditorJson(): JsonObject {
        val jsonObject = JsonObject()
        for (panel in Cue.hudEditor!!.panels) {
            val panelJson = JsonObject()
            panelJson.addProperty("IsShowModules", panel.isShowModules)
            panelJson.addProperty("X", panel.x)
            panelJson.addProperty("Y", panel.y)
    ...
    private fun saveFriends() {
        ...
        Files.write(
            friendFile.toPath(), Gson().toJson(Cue.friendManager!!.getAllFriend()).toByteArray(
                StandardCharsets.UTF_8
            )
        )
    }
    ...
    private fun getModulePath(module: dev.canon.cue.module.AbstractModule): File {
