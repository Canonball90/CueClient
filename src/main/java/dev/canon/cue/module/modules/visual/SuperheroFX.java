    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (mc.getRenderManager().options != null) {
            ...
            this.popTexts.forEach(pop -> {
                ...
                GlStateManager.translate(-((double) Cue.fontManager.badaboom.getStringWidth(pop.getDisplayName()) / 2.0), 0.0, 0.0);
                Cue.fontManager.badaboom.drawString(pop.getDisplayName(), 0, 0, pop.color);
                ...
                if (mc.world != null) {
                    ...
                    if (packet.getOpCode() == 35) {
                        if (mc.player.getDistance(e) < 20.0f) {
                            PopupText popupText = new PopupText(ChatFormatting.ITALIC + "POP", e.getPositionVector().add(this.rand.nextInt(2) / 2, 1.0, this.rand.nextInt(2) / 2));
                            popTexts.add(popupText);
                        }
