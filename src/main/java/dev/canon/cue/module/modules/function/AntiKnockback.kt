...
import dev.canon.cue.inject.client.ISPacketExplosion
        ...
        if (explo.value) {
            if (e.getPacket<Packet<*>>() is SPacketExplosion) {
                if (horiz.value == 0.0f && vert.value == 0.0f) {
                    ...
                    val motionX =
                        (packet as ISPacketExplosion).motionX / 100
                    val motionY =
                        (packet as ISPacketExplosion).motionY / 100
                    val motionZ =
                        (packet as ISPacketExplosion).motionZ / 100
                    (packet as ISPacketExplosion).motionX =
                        motionX * horiz.value
                    (packet as ISPacketExplosion).motionY =
                        motionY * vert.value
                    (packet as ISPacketExplosion).motionZ =
                        motionZ * horiz.value
                }
            }
        }
