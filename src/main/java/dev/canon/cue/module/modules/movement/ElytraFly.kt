...
import dev.canon.cue.event.events.player.TravelEvent
import dev.canon.cue.inject.client.IMinecraft
import dev.canon.cue.inject.client.ITimer
...
import dev.canon.cue.utils.Utils
...
class ElytraFly : Module() {
    ...
    override fun onEnable() {
        ...
        if (takeOff.value) {
            if (!mc.player.isElytraFlying) {
                ...
                ((mc as IMinecraft).hookGetTimer() as ITimer).hookSetTickLength(
                    50 / takeOffTimer.value
                )
    ...
    fun move(speed: Float) {
        val mover =
            if (Utils.mc.player.isRiding) Utils.mc.player.ridingEntity else Utils.mc.player
    ...
    fun forward(speed: Double): Vec3d {
        ...
        var playerYaw =
            Utils.mc.player.prevRotationYaw + (Utils.mc.player.rotationYaw - Utils.mc.player.prevRotationYaw) * Utils.mc.renderPartialTicks
    ...
    fun propel(speed: Float) {
        ...
        Utils.mc.player.motionX -= sin(Math.toRadians(yaw.toDouble())) * cos(
            Math.toRadians(
                pitch.toDouble()
            )
        ) * speed
        Utils.mc.player.motionZ += cos(Math.toRadians(yaw.toDouble())) * cos(
            Math.toRadians(
                pitch.toDouble()
            )
        ) * speed
    ...
    fun BlockPos.getBlockAtPos(): Block =
        Utils.mc.world.getBlockState(this).block
}