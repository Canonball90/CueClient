...
import dev.canon.cue.utils.render.Render2DUtil;
        ...
        for (Entity e : NameTags.mc.world.loadedEntityList) {
            ...
            if (background.getValue()) {
                if (gradientBackground.getValue()) {
                    Render2DUtil.drawGradientBorderedRect(-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2 - width.getValue(), -2 - height.getValue(), NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16 + width.getValue(), 10 + height.getValue(), new Color(25, 25, 25, 150).getRGB());
                } else {
                    Render2DUtil.drawBorderedRect(-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2 - width.getValue(), -2 - height.getValue(), NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16 + width.getValue(), 10 + height.getValue(), outlineWidth.getValue(), (background.getValue() ? new Color(25, 25, 25, 150).getRGB() : new Color(25, 25, 25, 0).getRGB()), ClickGui.getCurrentColor().getRGB());
                }

            } else if (roundedOutline.getValue()) {
                drawRoundedRect(-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2 - width.getValue(), -2 - height.getValue(), NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16 + width.getValue(), 10 + height.getValue(), new Color(25, 25, 25, 150).getRGB(), radius.getValue().intValue());
                drawOutlineRoundedRect(-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2 - width.getValue(), -2 - height.getValue(), NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16 + width.getValue(), 10 + height.getValue(), ClickGui.getCurrentColor().getRGB(), radius.getValue().intValue(), mc.player.getDistance(e), size);
            } else {
                Render2DUtil.drawBorderedRect(-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2 - width.getValue(), -2 - height.getValue(), NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16 + width.getValue(), 10 + height.getValue(), outlineWidth.getValue(), new Color(25, 25, 25, 0).getRGB(), ClickGui.getCurrentColor().getRGB());
            }
...
//            }
            if (cFont.getValue()) {
                Cue.fontManager.CustomFont.drawStringWithShadow(e.getName() + " " + TextFormatting.GREEN + health + (heart.getValue() ? "\u2764" : "%"), -this.getcenter(e.getName() + " " + TextFormatting.GREEN + health + "%") + cfontOffsetX.getValue(), 2, -1);
            } else {
                mc.fontRenderer.drawStringWithShadow(e.getName() + " " + TextFormatting.GREEN + health + (heart.getValue() ? "\u2764" : "%"), -this.getcenter(e.getName() + " " + TextFormatting.GREEN + health + "%"), 1, -1);
            }
            int posX = -NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 - 8;
            if (healthBar.getValue()) {
                Render2DUtil.drawLine((-NameTags.mc.fontRenderer.getStringWidth(e.getName() + " " + health + "%") / 2 - 2) - width.getValue(), 11 + height.getValue(), NameTags.mc.fontRenderer.getStringWidth(e.getName()) / 2 + 16 + width.getValue() + (-health), 11 + height.getValue(), 3, new Color(0, 255, 0));
            }
            if (items.getValue()) {
            ...
            for (ItemStack item : e.getArmorInventoryList()) {
                if (armor.getValue()) {
                    ...
                    for (Object enchantment : enchantments.keySet()) {
                        if (enchant.getValue()) {
                            int level = EnchantmentHelper.getEnchantmentLevel((Enchantment) enchantment, item);
