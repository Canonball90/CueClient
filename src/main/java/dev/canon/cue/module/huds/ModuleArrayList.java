...
public class ModuleArrayList extends HudModule {
    ...
    public void onRender2D() {
        count = 0;
        Cue.moduleManager.getModuleList().stream()
                .filter(AbstractModule::isEnabled)
                .filter(module -> module.visible.getValue())
                .sorted(Comparator.comparing(module -> Cue.fontManager.CustomFont.getStringWidth(module.getFullHud())
                        * (sortSetting.getValue().equals(sortMode.Bottom) ? 1 : -1)))
                .forEach(module -> {
                    float modWidth = Cue.fontManager.CustomFont.getStringWidth(module.getFullHud());
                    ...
                    if (colorSetting.getValue().equals(colorMode.Static) || colorSetting.getValue().equals(colorMode.CategoryC)) {
                        if (alignMode.Right.equals(alignSetting.getValue())) {
                            Cue.fontManager.CustomFont.drawStringWithShadow(modText,
        ...
        width = Cue.moduleManager.getModuleList().stream()
                .filter(AbstractModule::isEnabled)
                .noneMatch(module -> module.visible.getValue()) ? 20 :
                Cue.fontManager.CustomFont.getStringWidth(Cue.moduleManager.getModuleList()
                        .stream().filter(AbstractModule::isEnabled)
                        .filter(module -> module.visible.getValue())
                        .sorted(Comparator.comparing(module -> Cue.fontManager.CustomFont.getStringWidth(module.getFullHud()) * (-1)))
                        .collect(Collectors.toList()).get(0).getFullHud());
        height = ((Cue.fontManager.CustomFont.getHeight() + 1) *
                (int) Cue.moduleManager.getModuleList().stream()
