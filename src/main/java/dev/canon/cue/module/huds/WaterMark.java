...
public class WaterMark extends HudModule {
    ...
    public void onRender2D() {
        ...
        if (mode.getValue().equals(nfd.Logo)) {
            Cue.fontManager.IconFont.drawString("t", 0, 0, ClickGui.getCurrentColor().getRGB());
        }
        GL11.glPopMatrix();
        GL11.glPushMatrix();
        GL11.glTranslated(this.x, this.y, 0);
        GL11.glScaled(1, 1, 0.0);
        if (version.getValue()) {
            Cue.fontManager.CustomFont.drawString("ver:" + Cue.MOD_VERSION, 0 + width, 0 + height - 1, ClickGui.getCurrentColor().getRGB());
        }
        GL11.glPopMatrix();
    ...
    public static void drawFadeString(final String s, float x, float y) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            Cue.fontManager.CustomFont.drawString(str, (int) updateX, (int) y, color.hashCode());
